...
impl LlvmGenerator {
    pub(crate) fn ir_type_to_llvm(&self, ty: &IrType) -> String {
        match ty {
            ...
            IrType::String | IrType::Function(_, _) => "i8*".to_string(),
            IrType::Array(t, sz) => format!("[{} x {}]", sz, self.ir_type_to_llvm(t)),
        }
    }

    pub(crate) fn block_id_to_label(id: &str) -> String {
        format!("bb_{}", id.trim_start_matches("BB"))
    }

    pub(crate) fn get_extern_signature(name: &str) -> String {
        match name {
