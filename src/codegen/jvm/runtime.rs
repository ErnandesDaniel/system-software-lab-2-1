...
impl JvmGenerator {
    pub(super) fn generate_runtime_stub(&mut self, functions: &[IrFunction]) -> Vec<u8> {
        let this_class = self.constant_pool.add_class("RuntimeStub")
        ...
        let code_attr = self.constant_pool.add_utf8("Code")
            .expect("Failed to add 'Code' UTF8");
        let coro_field_name = self.constant_pool.add_utf8("coroutines")
            .expect("Failed to add 'coroutines' UTF8");
        let coro_field_desc = self.constant_pool.add_utf8("[Ljava/lang/Object;")
            .expect("Failed to add coro field descriptor");
        let coro_field_ref = self.constant_pool.add_field_ref(this_class, "coroutines", "[Ljava/lang/Object;")
        ...
        let coro_info: Vec<(String, u16)> = functions.iter()
            ...
            .map(|f| {
                ...
                let class_idx = self.constant_pool.add_class(&name)
                    .expect("Failed to add coroutine class");
                let _ = self.constant_pool.add_method_ref(class_idx, "<init>", "()V")
                    .expect("Failed to add coroutine init ref");
                let _ = self.constant_pool.add_method_ref(class_idx, "resume", "()I")
    ...
    fn build_coro_resume(
        ...
        let resume_name = self.constant_pool.add_utf8("resume_coroutine")
            .expect("Failed to add 'resume_coroutine' UTF8");
        let resume_desc = self.constant_pool.add_utf8("(I)I")
        ...
        for (i, (_, ci)) in coro_info.iter().enumerate() {
            ...
            let rm = self.constant_pool.add_method_ref(*ci, "resume", "()I")
    ...
    fn build_coro_get_state(
        ...
        let state_name = self.constant_pool.add_utf8("get_coroutine_state")
            .expect("Failed to add 'get_coroutine_state' UTF8");
        let state_desc = self.constant_pool.add_utf8("(I)I")
        ...
        for (i, (_, ci)) in coro_info.iter().enumerate() {
            ...
            let sm = self.constant_pool.add_method_ref(*ci, "getState", "()I")
    ...
    fn build_coro_set_param(
        ...
        let set_name = self.constant_pool.add_utf8("set_coroutine_param")
            .expect("Failed to add 'set_coroutine_param' UTF8");
        let set_desc = self.constant_pool.add_utf8("(III)V")
