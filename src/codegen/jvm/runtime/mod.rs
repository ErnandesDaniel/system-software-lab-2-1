...
use ristretto_classfile::{
    ClassAccessFlags, ClassFile, Field, FieldAccessFlags, FieldType, Method, MethodAccessFlags,
};
...
impl JvmGenerator {
    pub(super) fn generate_runtime_stub(&mut self, functions: &[IrFunction]) -> Vec<u8> {
        ...
        let coro_field_ref = self.pool.constant_pool
            .add_field_ref(this_class, "coroutines", "[Ljava/lang/Object;").unwrap();
        ...
        let system_out_ref = self.pool.constant_pool
            .add_field_ref(system_class, "out", "Ljava/io/PrintStream;").unwrap();
        let printstream_class = self.pool.constant_pool.add_class("java/io/PrintStream").unwrap();
        let print_char_ref = self.pool.constant_pool
            .add_method_ref(printstream_class, "print", "(C)V").unwrap();
        let println_string_ref = self.pool.constant_pool
            .add_method_ref(printstream_class, "println", "(Ljava/lang/String;)V").unwrap();
        ...
        let string_byte_init = self.pool.constant_pool
            .add_method_ref(string_class, "<init>", "([B)V").unwrap();
        ...
        let file_fds_ref = self.pool.constant_pool
            .add_field_ref(this_class, "fileStreams", "[Ljava/io/InputStream;").unwrap();
        let file_next_ref = self.pool.constant_pool
            .add_field_ref(this_class, "fileNext", "I").unwrap();
        let fis_class = self.pool.constant_pool.add_class("java/io/FileInputStream").unwrap();
        let fis_init = self.pool.constant_pool
            .add_method_ref(fis_class, "<init>", "(Ljava/lang/String;)V").unwrap();
        let is_class = self.pool.constant_pool.add_class("java/io/InputStream").unwrap();
        let is_read = self.pool.constant_pool
            .add_method_ref(is_class, "read", "()I").unwrap();
        let is_close = self.pool.constant_pool
            .add_method_ref(is_class, "close", "()V").unwrap();
        let int_class = self.pool.constant_pool.add_class("java/lang/Integer").unwrap();
        let int_parse = self.pool.constant_pool
            .add_method_ref(int_class, "parseInt", "(Ljava/lang/String;)I").unwrap();
        let str_init_3arg = self.pool.constant_pool
            .add_method_ref(string_class, "<init>", "([BII)V").unwrap();

        self.build_nullscan(&mut methods, code_attr, this_class);
        self.build_fopen(&mut methods, code_attr, string_class, str_init_3arg,
            fis_class, fis_init, file_fds_ref, file_next_ref, is_class);
        self.build_puts(&mut methods, code_attr, system_out_ref,
            string_class, str_init_3arg, println_string_ref);
        ...
        self.build_printf_methods(&mut methods, code_attr, string_class,
            string_byte_init, str_init_3arg, system_out_ref, printstream_class, int_class);
            ...
            if let Some(&fr) = self.global.global_field_refs.get(gname) {
                if self.global.global_uses_object_array.contains(gname) {
                    ...
                    if self.pool.object_class_idx == 0 {
                        self.pool.object_class_idx = self.pool.constant_pool
                            .add_class("java/lang/Object").unwrap();
                    }
                    ...
                    if *n > 0 {
                        let sz = *n as i16;
                        clinit_code.push(
                            if sz <= 127 { Instruction::Bipush(sz as i8) } else { Instruction::Sipush(sz) }
                        );
                    ...
                    clinit_code.push(
                        if size <= 127 { Instruction::Bipush(size as i8) } else { Instruction::Sipush(size) }
                    );
                    clinit_code.push(Instruction::Newarray(ristretto_classfile::attributes::ArrayType::Int));
                    clinit_code.push(Instruction::Putstatic(fr));
                }
        ...
        methods.push(Method {
            ...
            attributes: vec![Attribute::Code {
                name_index: code_attr, max_stack: 3, max_locals: 0,
                code: clinit_code,
        ...
        if count > 0 {
            fields.push(Field {
                ...
                name_index: coro_field_name, descriptor_index: coro_field_desc,
        ...
        fields.push(Field {
            access_flags: FieldAccessFlags::PUBLIC | FieldAccessFlags::STATIC,
            name_index: file_fds_name, descriptor_index: file_fds_desc,
            field_type: FieldType::parse("[Ljava/io/InputStream;").unwrap(),
            attributes: vec![],
        });
        fields.push(Field {
            access_flags: FieldAccessFlags::PUBLIC | FieldAccessFlags::STATIC,
            name_index: file_next_name, descriptor_index: file_next_desc,
        ...
        let class_file = ClassFile {
            ...
            fields, methods, attributes: vec![], code_source_url: None,
        };
        let mut buf = Vec::new();
    ...
    fn build_nullscan(&mut self, methods: &mut Vec<Method>, code_attr: u16, this_class: u16) {
        ...
        methods.push(Method {
            ...
            name_index: n, descriptor_index: d,
            attributes: vec![Attribute::Code {
                name_index: code_attr, max_stack: 3, max_locals: 3,
                code: vec![
                    Instruction::Iconst_0, Instruction::Istore(1),
                    Instruction::Goto(4), Instruction::Iinc(1, 1),
                    Instruction::Iload(1), Instruction::Aload_0,
                    Instruction::Arraylength, Instruction::If_icmpge(13),
                    Instruction::Aload_0, Instruction::Iload(1),
                    Instruction::Baload, Instruction::Ifeq(13),
                    Instruction::Goto(3), Instruction::Iload(1),
                    Instruction::Ireturn,
                ],
                exception_table: vec![], attributes: vec![],
            }],
        });
        self.pool.nullscan_ref = self.pool.constant_pool
            .add_method_ref(this_class, "nullscan", "([B)I").unwrap();
    }

    fn build_fopen(&mut self, methods: &mut Vec<Method>, code_attr: u16, string_class: u16, str_init_3arg: u16, fis_class: u16, fis_init: u16, file_fds_ref: u16, file_next_ref: u16, _is_class: u16) {
        ...
        methods.push(Method {
            ...
            name_index: n, descriptor_index: d,
            attributes: vec![Attribute::Code {
                name_index: code_attr, max_stack: 5, max_locals: 4,
                code: vec![
                    Instruction::Aload_0,
                    Instruction::Invokestatic(self.pool.nullscan_ref),
                    Instruction::Istore(3),
                    Instruction::New(string_class), Instruction::Dup,
                    Instruction::Aload_0, Instruction::Iconst_0,
                    Instruction::Iload(3),
                    Instruction::Invokespecial(str_init_3arg),
                    Instruction::Astore(2),
                    Instruction::New(fis_class), Instruction::Dup,
                    Instruction::Aload_2,
                    Instruction::Invokespecial(fis_init),
                    Instruction::Astore_2,
                    Instruction::Getstatic(file_fds_ref),
                    Instruction::Getstatic(file_next_ref),
                    Instruction::Aload_2, Instruction::Aastore,
                    Instruction::Getstatic(file_next_ref),
                    Instruction::Dup, Instruction::Iconst_1,
                    Instruction::Iadd,
                    Instruction::Putstatic(file_next_ref),
                    Instruction::Iconst_1, Instruction::Iadd,
                    Instruction::Ireturn,
                ],
                exception_table: vec![], attributes: vec![],
            }],
    ...
    fn build_fgetc(&mut self, methods: &mut Vec<Method>, code_attr: u16, file_fds_ref: u16, is_class: u16, is_read: u16) {
        ...
        methods.push(Method {
            ...
            name_index: n, descriptor_index: d,
            attributes: vec![Attribute::Code {
                name_index: code_attr, max_stack: 2, max_locals: 2,
                code: vec![
                    Instruction::Iload_0, Instruction::Iconst_1,
                    Instruction::Isub, Instruction::Istore(1),
                    Instruction::Getstatic(file_fds_ref),
                    Instruction::Iload(1), Instruction::Aaload,
    ...
    fn build_fclose(&mut self, methods: &mut Vec<Method>, code_attr: u16, file_fds_ref: u16, is_class: u16, is_close: u16) {
        ...
        methods.push(Method {
            ...
            name_index: n, descriptor_index: d,
            attributes: vec![Attribute::Code {
                name_index: code_attr, max_stack: 3, max_locals: 2,
                code: vec![
                    Instruction::Iload_0, Instruction::Iconst_1,
                    Instruction::Isub, Instruction::Istore(1),
                    Instruction::Getstatic(file_fds_ref),
                    ...
                    Instruction::Getstatic(file_fds_ref),
                    Instruction::Iload(1),
                    Instruction::Aconst_null, Instruction::Aastore,
                    Instruction::Iconst_0, Instruction::Ireturn,
                ],
                exception_table: vec![], attributes: vec![],
            }],
        });
