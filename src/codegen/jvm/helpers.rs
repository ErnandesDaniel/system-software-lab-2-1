...
impl JvmGenerator {
    ...
    pub fn get_field_slot_for_offset(&self, var_name: &str, byte_off: usize) -> usize {
        if let Some(fields) = self.struct_field_types.get(var_name) {
    ...
    pub fn get_global_object_array_inner_size(&self, name: &str) -> usize {
        if let Some(offsets) = self.global_struct_offset_sets.get(name) {
            offsets.len()
        } else if let Some(ty) = self.global_vars.get(name) {
    ...
    pub fn is_struct_var(&self, operand: &IrOperand) -> bool {
        if let IrOperand::Variable(name, _) = operand {
            self.struct_uses_object_array.contains(name) || self.global_uses_object_array.contains(name)
        } else {
            false
        }
    }
