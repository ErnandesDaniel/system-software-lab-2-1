...
#[test]
fn test_until_loop() {
    let source = "def foo() { x = 0; until (x == 5) { x = x + 1; } }";
    let program = parse(source);
...
fn test_do_while_loop() {
    let source = "def foo() { x = 0; { x = x + 1; } while (x < 5); }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_break_statement() {
    let source = "def foo() { while (1) { break; } }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_comparison_operators() {
    let source = "def foo() { x = 1 == 2; y = 3 != 4; z = 5 < 6; w = 7 > 8; a = 9 <= 10; b = 11 >= 12; }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_logical_operators() {
    let source = "def foo() { x = flag1 && flag2; y = flag3 || flag4; }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_var_declaration() {
    let source = "def foo() { x of int; y of string; z of bool; }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_array_type_var() {
    let source = "def foo() { arr of int[10]; }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
...
fn test_bool_literal() {
    let source = "def foo() { x = true; y = false; }";
    let program = parse(source);
    assert_eq!(program.items.len(), 1);
}
