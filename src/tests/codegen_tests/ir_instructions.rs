...
fn test_ir_load_or_assign_instruction() {
    ...
    let has_load_or_assign = ir.functions[0]
        .blocks
        .iter()
        .flat_map(|b| &b.instructions)
...
fn test_ir_assign_or_store_instruction() {
    ...
    let has_assign_or_store = ir.functions[0]
        .blocks
        .iter()
        .flat_map(|b| &b.instructions)
...
fn test_ir_while_loop_instructions() {
    ...
    let has_cmp = ir.functions[0].blocks.iter().flat_map(|b| &b.instructions).any(|i| {
        matches!(
            i.opcode,
            crate::ir::IrOpcode::Lt | crate::ir::IrOpcode::Le | crate::ir::IrOpcode::Ge | crate::ir::IrOpcode::Gt
        )
    });
...
fn test_ir_field_access_load() {
    ...
    assert!(
        has_load >= 1,
        "Expected at least 1 Load for field access, got {has_load}"
    );
}
...
fn test_ir_return_operand() {
    ...
    let has_var_or_const = rets.iter().any(|i| {
        i.operands.iter().any(|o| {
            matches!(
                o,
                crate::ir::IrOperand::Variable(_, _) | crate::ir::IrOperand::Constant(_)
            )
        })
    });
...
fn test_ir_return_variable_operand() {
    ...
    let has_var = rets.iter().any(|i| {
        i.operands
            .iter()
            .any(|o| matches!(o, crate::ir::IrOperand::Variable(_, _)))
    });
